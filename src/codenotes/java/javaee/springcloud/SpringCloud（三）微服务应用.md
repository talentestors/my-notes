...
![image-20230306234928969](./img/hV2JkERda4qKtjB.png)
...
![image-20230306234940672](./img/pqZolFN6eIPza52.png)
...
![image-20230306234948408](./img/FYcNvAuZ7z8rj2V.png)
...
![image-20230306234959085](./img/SyCJXKgO3qGx8EL.png)

可以看到三个服务都正常注册了，接着我们去访问图书服务：

![image-20230306235007563](./img/gytqnZjTMvVEUm3.png)
...
![image-20230306235015909](./img/xfV5oYGvc1jKqTM.png)

同时用户服务也能正常访问了：

![image-20230306235021694](./img/OH6wjLVreot4IiA.png)

我们可以查看一下Redis服务器中是不是存储了我们的Session信息：

![image-20230306235046117](./img/nNIkoXOAYuMH8aV.png)

虽然看起来好像确实没啥问题了，但是借阅服务炸了，我们来看看为什么：

![image-20230306235053840](./img/wls5vCajnuMBOkU.png)
...
![image-20230306235102514](./img/46ukOAiDzMZBX15.png)
...
1. **客户端模式（Client Credentials）**
   ...
   ![image-20230306235141113](./img/4i16wzqOnYeaB2c.png)
...
2. **密码模式（Resource Owner Password Credentials）**
   ...
   ![image-20230306235151635](./img/JEreS9nQD8ojMca.png)
...
3. **隐式授权模式（Implicit Grant）**
   ...
   ![image-20230306235200365](./img/MRxnKyWT3br5Zj2.png)
...
4. **授权码模式（Authrization Code）**
   ...
   ![image-20230306235211335](./img/2EIPfirBOKbcndk.png)
...
![image-20230306235309409](./img/B9TzojnUq4KvVPr.png)

可以看到active为true，表示我们刚刚申请到的Token是有效的。

接着我们来测试一下第二种password模式，我们还需要提供具体的用户名和密码，授权模式定义为password即可：

![image-20230306235318785](./img/jt5XPZKvRFqr73x.png)

接着我们需要在请求头中添加Basic验证信息，这里我们直接填写id和secret即可：

![image-20230306235327745](./img/K9ZpIv8SzcfsHd4.png)

可以看到在请求头中自动生成了Basic验证相关内容：

![image-20230306235335919](./img/JHxPKgFU5wY7SB8.png)

![image-20230306235345388](./img/F3WU7XhqridywVn.png)

响应成功，得到Token信息，并且这里还多出了一个refresh_token，这是用于刷新Token的，我们之后会进行讲解。

![image-20230306235355181](./img/zjuc2qxQmBas5r1.png)
...
![image-20230306235441824](./img/OYeRQpEXFSoZMhc.png)

但是登录之后我们发现出现了一个错误：

![image-20230306235501474](./img/qLUkJFZau8eQ6WO.png)
...
![image-20230306235536317](./img/da4WseDt172hbLV.png)
...
![image-20230306235545717](./img/e1Zdt9IP7vp2zMO.png)

可以看到结果也是正常返回了Token信息：

![image-20230306235554061](./img/qY5kxgBWSzMJXco.png)

这样我们四种最基本的Token请求方式就实现了。

最后还有一个是刷新令牌使用的，当我们的Token过期时，我们就可以使用这个refresh_token来申请一个新的Token：

![image-20230306235603731](./img/d2ojclCLB3mQu7D.png)

但是执行之后我们发现会直接出现一个内部错误：

![image-20230306235611413](./img/BcFMIg4NqCx8kdh.png)

![image-20230306235618838](./img/cA9WF1KxyUDZ8Bi.png)
...
![image-20230306235638503](./img/QWEwzpiq7FXnv3f.png)
...
![image-20230306235651433](./img/DrVSZtdKNCoMucx.png)

可以看到在发现没有登录验证时，会直接跳转到授权页面，进行授权登录，之后才可以继续访问图书服务：

![image-20230306235701725](./img/nsJGmxcOVYXDUqd.png)
...
![image-20230306235733777](./img/y1VYRC9tmOv854u.png)
...
![image-20230306235744660](./img/7zbqlOrSCVRdQ4y.png)
...
![image-20230306235756676](./img/QiZmqznyMxNpETk.png)
...
![image-20230306235804196](./img/Np6PKCZD2kAdmtf.png)

另一种我们需要使用Postman来完成：

![image-20230306235814228](./img/ypR3G7DxsYicMQI.png)

添加验证信息后，会帮助我们转换成请求头信息：

![image-20230306235826416](./img/qPHDU1dXgC7srn3.png)

![image-20230306235833330](./img/6IeMvTcCKdfbUlV.png)
...
![image-20230306235844783](./img/5T4d39YkcZIomvD.png)
...
![image-20230306235853397](./img/k3LmR9E7UBtVA5x.png)
...
![image-20230306235903496](./img/mvKqyJk7P1FCQSl.png)
...
![image-20230306235926214](./img/BqkomFVGK7wv64X.png)
...
![image-20230306235934751](./img/PZkS8GyU1jhpIrz.png)
...
![image-20230306235953555](./img/zgEJF7AeHw8iGIZ.png)
...
![image-20230307000004710](./img/Xu8lxYhKoJNr6it.png)
...
![image-20230307000016944](./img/W95CFKAmd1wfgSJ.png)
...
![image-20230307000026772](./img/C6OteoFghrxpYjQ.png)

可以看到成功获取了AccessToken，但是这里的格式跟我们之前的格式就大不相同了，因为现在它是JWT令牌，我们可以对其进行一下Base64解码：

![image-20230307000035581](./img/CUMkrRfgOthZKVz.png)
...
![image-20230307000043443](./img/kOpRlTB7SPtQa4y.png)

请求成功，得到数据：

![image-20230307000053404](./img/aicW89KezTSZ7f5.png)
...
![image-20230307000109568](./img/bzwPflgBD5O1saN.png)
...
![image-20230307000216155](./img/taxoisA8Tpg2DWM.png)

可以看到，我们在6001服务器插入的`a`，可以在从节点6002读取到，那么，从节点新增的数据在主节点能得到吗？我们来测试一下：

![image-20230307000224291](./img/dS2V8xafPj6lKND.png)

可以看到，从节点压根就没办法进行数据插入，节点的模式为只读模式。那么如果我们现在不想让6002作为6001的从节点了呢？

![image-20230307000234718](./img/dV7Rxov6pblW2g5.png)

可以看到，通过输入`replicaof no one`，即可变回Master角色。接着我们再来启动一台6003服务器，流程是一样的：

![image-20230307000406674](./img/TC7z2mt3EGMPWfq.png)
...
![image-20230307000415001](./img/MmNshyQxa2ijSRT.png)

但是从节点会疯狂报错：

![image-20230307000424822](./img/pEIo93MQXShrsZD.png)
...
![image-20230307000432434](./img/GpAa5kfyC3zVRZK.png)

当然，除了作为Master节点的从节点外，我们还可以将其作为从节点的从节点，比如现在我们让6003作为6002的从节点：

![image-20230307000444990](./img/OdAs1weYgkDrQvf.png)

也就是说，现在差不多是这样的的一个情况：

![image-20230307000459161](./img/2ADSR8LtpMhCFfK.png)
...
![image-20230307000508084](./img/YGq8MDZbRK6E7Po.png)
...
![image-20230307000516875](./img/WhkUqfxcHn4CApP.png)
...
![image-20230307000534399](./img/xB78t53RgykXvo9.png)

![image-20230307000542878](./img/STh2RgjW7ycPCNB.png)

可以看到以哨兵模式启动后，会自动监控主节点，然后还会显示那些节点是作为从节点存在的。

现在我们直接把主节点关闭，看看会发生什么事情：

![image-20230307000627807](./img/97HnwfuNjUK5qx4.png)

可以看到从节点还是正常的在报错，一开始的时候不会直接重新进行选举而是继续尝试重连（因为有可能只是网络小卡一下，没必要这么敏感），但是我们发现，经过一段时间之后，依然无法连接，哨兵输出了以下内容：

![image-20230307000646050](./img/GWt8Q6mfSv7TgCb.png)

可以看到哨兵发现主节点已经有一段时间不可用了，那么就会开始进行重新选举，6003节点被选为了新的主节点，并且之前的主节点6001变成了新的主节点的从节点：

![image-20230307000653822](./img/4WzTVZ15dMiQ3f8.png)

![image-20230307000703587](./img/gGHVvOhBKe9wSEz.png)

当我们再次启动6001时，会发现，它自动变成了6003的从节点，并且会将数据同步过来：

![image-20230307000717002](./img/eqLycu8s1rSRtFa.png)
...
![image-20230307000731297](./img/48MNiLJXqmUtvWc.png)

现在我们把6001节点挂掉，看看这三个哨兵会怎么样：

![image-20230307000741214](./img/ajSAhqb5L9Yuorg.png)
...
![image-20230307000754039](./img/TjCw8DLi1VqYvpZ.png)
...
![image-20230307000805968](./img/7DikHoxKJPve9qa.png)

输入之后，会为你展示客户端默认分配的方案，并且会询问你当前的方案是否合理。可以看到6001/6002/6003都被选为主节点，其他的为从节点，我们直接输入yes即可：

![image-20230307000814076](./img/yxZhjouXB79qfDd.png)

最后分配成功，可以看到插槽的分配情况：

![image-20230307000830886](./img/8kg9TbadF2qyHJW.png)

现在我们随便连接一个节点，尝试插入一个值：

![image-20230307000840119](./img/YMf8DtlkCsqBpO1.png)

在插入时，出现了一个错误，实际上这就是因为a计算出来的哈希值（插槽），不归当前节点管，我们得去管这个插槽的节点执行，通过上面的分配情况，我们可以得到15495属于节点6003管理：

![image-20230307000849127](./img/EZmR2bLFudSskIf.png)

在6003节点插入成功，当然我们也可以使用集群方式连接，这样我们无论在哪个节点都可以插入，只需要添加`-c`表示以集群模式访问：

![image-20230307000907196](./img/mVw7EXFJQOcinIb.png)

可以看到，在6001节点成功对a的值进行了更新，只不过还是被重定向到了6003节点进行插入。

我们可以输入`cluster nodes`命令来查看当前所有节点的信息：

![image-20230307000914746](./img/pEJdI2UWTcNZFqu.png)

那么现在如果我们让某一个主节点挂掉会怎么样？现在我们把6001挂掉：

![image-20230307000922277](./img/zd6L3WosVE8JUqf.png)

可以看到原本的6001从节点7001，晋升为了新的主节点，而之前的6001已经挂了，现在我们将6001重启试试看：

![image-20230307000933812](./img/eUfYJS8yhVijvaw.png)

可以看到6001变成了7001的从节点，那么要是6001和7001都挂了呢？

![image-20230307000944827](./img/9W2BQtMXrUCnySV.png)

这时我们尝试插入新的数据：

![image-20230307000953377](./img/S8r5TE7gJ3M6iDW.png)

可以看到，当存在节点不可用时，会无法插入新的数据，现在我们将6001和7001恢复：

![image-20230307001001435](./img/2RL4GNqSWJXFuME.png)
...
![image-20230307001013418](./img/fNCxEJRX61cPsuk.png)

可以看到，当客户端1设定a之后，客户端2使用`setnx`会直接失败。

![image-20230307001022446](./img/wpGutcmxEsWFJVn.png)
...
![image-20230307001031311](./img/eQEIGKONmkB2u6y.png)

当然，添加了过期时间，带了的好处是显而易见的，但是同时也带来了很多的麻烦，我们来设想一下这种情况：

![image-20230307001038169](./img/nStuP75RLOmQWUM.png)
...
![image-20220414113041835](./img/4DW1K38UqQJdwkf.jpg)
...
![image-20220414113709773](./img/8I1Atm7BOZC5ifS.jpg)
...
![image-20220414133258227](./img/y2Nvi816ut4jpGC.jpg)
...
![image-20220414133403403](./img/Gyz1Rc7OWhT5NJK.jpg)
...
![image-20220414162319865](./img/95wL8vICYNp61T2.jpg)
...
![image-20220414164943974](./img/kqHZoc8xAbNOd3K.jpg)
...
![image-20220414170022303](./img/H7BIl9s3kPu2Mnw.jpg)
...
![image-20220414192045320](./img/KiCoVP1cGaf94uX.jpg)
...
![image-20220414192508849](./img/RxNB3QmUYESX5ad.jpg)
...
![image-20220414192829536](./img/qKBwz31P6ySxlZt.jpg)

现在我们随便插入一点数据：

![image-20220414192920277](./img/9pqBFXiLhTPc2xO.jpg)

这样，我们的MySQL主从就搭建完成了，那么如果主机此时挂了会怎么样？

![image-20220414200140191](./img/s1Q5xt32r6dv9UJ.jpg)
...
![image-20220414212443482](./img/GEfPLSIZyobhtTe.jpg)
...
![image-20220414222958901](./img/Hvm82dfbEtwBqrA.jpg)
...
![image-20220415104401263](./img/7oBrFRwiXQxcumz.jpg)

测试通过，我们来看看数据库里面是不是按照我们的规则进行数据插入的：

![image-20220415104449502](./img/kZINi9wmnte3J7g.jpg)

可以看到这两张表，都成功按照我们指定的路由规则进行插入了，我们来看看详细的路由情况，通过控制台输出的SQL就可以看到：

![image-20220415105325917](./img/2e9cd91031d3fc7d2f11a2f59d8841ae.png)
...
![image-20220415110322981](./img/InHsNXA3E8dQBPa.jpg)
...
![image-20220415112809843](./img/OaRCMTJ1lnIicSd.jpg)

可以看到，根据我们的算法，原本的逻辑表被修改为了最终进行分表计算后的结果，我们来查看一下数据库：

![image-20220415112908760](./img/lfvgOjanPZHMNdr.jpg)
...
![image-20220415113139917](./img/7K1WBk3s8HuMeOI.jpg)
...
![image-20220415113530971](./img/3Hj7s4xqEwiFXJB.jpg)
...
![image-20220415113652038](./img/WoQqNLCXJslBT3D.jpg)
...
2. **雪花算法（Snowflake）：**
   ...
   ![image-20220415150713707](./img/lU9A4zjSIKvaxwh.jpg)
...
![image-20220415154524545](./img/2JBaqnV8k9OWYfw.jpg)
...
![image-20220415155842834](./img/KRBbGXxhkmUHFIr.jpg)
...
![image-20220415160249024](./img/8o4YIB5MysaUuFx.jpg)
...
![image-20220415162741466](./img/zJvqKmfyhVMFLtZ.jpg)
