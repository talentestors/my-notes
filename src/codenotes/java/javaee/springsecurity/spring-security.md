> [!TIP]
>
> 视频链接：<https://www.bilibili.com/video/BV1fV411M7aS>
...
![image-20230701174518205](./img/W3oCIbqvLUn492J.png)
...
![image-20230701175101126](./img/wBdynQcKU2g7LjN.png)
...
![image-20230701175235964](./img/5aHJDMTiLOSzERn.png)
...
![image-20230701184920238](./img/7jW5Nzki8urUhaf.png)
...
![image-20230701224909319](./img/khHj6YqSJimGKec.png)

### CSRF跨站请求伪造攻击

CSRF是我们要介绍的第一种攻击形式，这种攻击方式非常好理解。

![image-20230701181244150](./img/4ibrwFIPnSE81lx.png)
...
![image-20230701233127287](./img/4wQ2iB5uhcLMJHa.png)

接着我们假装自己上当，进入到我们的钓鱼网站：

![image-20230701233202016](./img/OstNZg4doCz6S5G.png)

现在我们毫不知情，如果是正常人思维的话，就会直接点击下载全套视频，恭喜，此时后台已经转账成功了，留下一脸懵逼的你：

![image-20230701233317019](./img/mtGLhuNHxPUr6Os.png)
...
![a3421f74c3cee6b67d214809adb743a5](./img/qiLDnrFyQxpt3UB.png)
...
![image-20230702001501063](./img/LFchsWkevUwb58E.png)
...
![image-20230702001831904](./img/d4YSeut3xQ7rRqh.png)

可以看到此时用户的浏览器JSESSIONID值为刚刚恶意网站伪造的值，现在我们来进行登录操作：

![image-20230702002007450](./img/aDwTcW6d58tPLBG.png)
...
![image-20230702002624508](./img/8IaHk2FEMwyoXem.png)
...
![image-20230702003831742](./img/jTcOaNdwDeP9qB2.png)
...
![image-20230702004827194](./img/DkzJWPxQ5BUl2tC.png)
...
![image-20230702135644834](./img/dWkGc5YhNAIbP8j.png)
...
![image-20230702144938540](./img/tSGxZmv6jUDMy95.png)

登录成功后，就可以访问到我们之前的界面了：

![image-20230702145011992](./img/Z8fxKehX26AMaJI.png)

并且为了防止我们之前提到的会话固定问题，在登录之后，JSESSIONID会得到重新分配：

![image-20230703192441811](./img/mQpWZMljCt2XTd7.png)

当我们想要退出时，也可以直接访问：http://localhost:8080/mvc/logout 地址，我们会进入到一个退出登录界面：

![image-20230702145432892](./img/iHQy63RxgUkvKsw.png)
...
![image-20230702151123338](./img/yuYe5pODwqBTQh7.png)
...
![image-20230702152150157](./img/Vacp97MlgfNrYnR.png)

![image-20230702152216162](./img/tk5pGDrNHWfaJXU.png)

这样，一个简单的权限校验就配置完成了，是不是感觉用起来还是挺简单的？

不过，可能会有小伙伴发现，所有的POST请求都被403了：

![image-20230702183040922](./img/F94URzLh6oIBrCJ.png)
...
[官方默认提供了可以直接使用的用户和权限表设计](https://docs.spring.io/spring-security/reference/servlet/authentication/passwords/jdbc.html)，根本不需要我们来建表，直接在Navicat中执行以下查询：
...
![image-20230702181131252](./img/VG19mSConefsilH.png)

![image-20230702181119809](./img/6uqerwFo13p9jxJ.png)

我们可以直接尝试进行登录，使用方式和之前是完全一样的：

![image-20230702181211157](./img/dVM5ltzF1ua8Y3E.png)
...
![image-20230703001525592](./img/akAtDrPeMdc6N3b.png)
...
![image-20230703181046326](./img/ln4uZ1TFIe7qaCK.png)

随便插入一点数据：

![image-20230703152719655](./img/tToR2JPykeuCK73.png)
...
![image-20230703180438190](./img/hpZs1DLESojHJue.png)
...
![image-20230703182321093](./img/Zns4Vwb7zPLc6SQ.png)
...
![image-20230703184425313](./img/c38kewdxtn1j2V6.png)

但是我们发现，我们的页面只有一个纯文本，这是因为在获取静态资源的时候，所有的静态资源默认情况下也会被拦截，因此全部被302重定向到登录页面，这显然是不对的：

![image-20230703184641792](./img/6vXlPZprzjJLEeq.png)
...
![image-20230703185027927](./img/LmZbihzD4vYB5GF.png)
...
![image-20230703185916404](./img/P2LS8uNRQ64WEvT.png)
...
![image-20230703190714519](./img/yM8TOAxYPf3iqFs.png)
...
![image-20230704211415804](./img/3wOt7CldbFP8yHz.png)

此时提交的表单中就已经包含记住我字段了，我们会发现，服务端返回给我们了一个记住我专属的Cookie信息：

![image-20230704211611369](./img/NB129h7IKRycXvL.png)
...
![image-20230704220701000](./img/kIJpuWdiEGqUKBx.png)
...
![image-20230704222032360](./img/e1IXMRgawYoGvSQ.png)
...
![image-20230704222733082](./img/1pkfGS9LrsPtjFx.png)
...
![image-20230704223248124](./img/8aoGrM9mpYt6Xie.png)

这是因为我们前面配置的是user角色，那么这个角色只能访问首页，其他的都不行，所以就会被自动拦截掉了。现在我们可以到数据库中对这个用户的角色进行修改，看看修改后是否能够访问到其他页面：

![image-20230704223503682](./img/l9YkDaRJdtrmSZj.png)
...
![image-20230705005303736](./img/LVxihksHZu2qN6X.png)
...
![image-20230706215806040](./img/uPjdsgbhv9NqA8B.png)
