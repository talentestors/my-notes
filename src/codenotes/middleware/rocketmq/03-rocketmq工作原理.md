...
2)读操作：若用户要读取数据，其首先会从PageCache中读取，若没有命中，则OS在从物理磁盘上加载该数据到PageCache的同时，也会顺序对其相邻数据块中的数据进行预读取。
...
> 由于可以推算出，一个indexFile的最大大小是：(40 + 500w _ 4 + 2000w _ 20)字节
...
> 500w \* 4是所有slots所占的字节数

#### 具体查询流程如下：

1. 根据查询的key计算hash值，然后根据式子1计算slot槽位序号
2. 根据式子2计算出该slot在indexFile中的位置
3. 从该位置读取到indexNo
4. 根据式子3计算出该indexNo对应的index索引单元在indexFile中的位置
5. 从该位置读取到phyOffset，即可定位到消息在commitlog中的位置
6. 若该slot下还有更多的index，则通过preIndexNo继续遍历

## 消息的消费

消费者从Broker中获取消息的方式有两种：pull拉取方式和push推动方式。消费者分组(ConsumerGroup)对于消息消费的模式又分为两种：集群消费Clustering和广播消费Broadcasting。
...
![消息粒度负载均衡](https://rocketmq.apache.org/zh/assets/images/clustermode-dfd781d08bc0c69111841bda537aa302.png)
...
由于一个队列最多分配给一个消费者，因此当某个消费者分组(ConsumerGroup)下的消费者实例数量大于队列的数量时，多余的消费者实例将分配不到任何队列。
...
导致Rebalance产生的原因，无非就两个：消费者所订阅Topic的Queue数量发生变化，或消费者分组(ConsumerGroup)中消费者的数量发生变化。
...
在Broker中维护着多个Map集合，这些集合中动态存放着当前Topic中Queue的信息、Consumer Group中Consumer实例的信息。一旦发现消费者所订阅的Queue数量发生变化，或消费者分组(ConsumerGroup)中消费者的数量发生变化，立即向Consumer Group中的每个实例发出Rebalance通知。
...
其可以有效减少由于消费者分组(ConsumerGroup)扩容或缩容所带来的大量的Rebalance。
...
订阅关系的一致性指的是，同一个消费者分组(ConsumerGroup)（Group ID相同）下所有Consumer实例所订阅的Topic与Tag及对消息的处理逻辑必须完全一致。否则，消息消费的逻辑就会混乱，甚至导致消息丢失。

#### 正确订阅关系

多个消费者分组(ConsumerGroup)订阅了多个Topic，并且每个消费者分组(ConsumerGroup)里的多个消费者实例的订阅关系保持了一致。

#### 错误订阅关系

一个消费者分组(ConsumerGroup)订阅了多个Topic，但是该消费者分组(ConsumerGroup)里的多个Consumer实例的订阅关系并没有保持一致。

#### 订阅了不同Topic

该例中的错误在于，同一个消费者分组(ConsumerGroup)中的两个Consumer实例订阅了不同的Topic。
...
该例中的错误在于，同一个消费者分组(ConsumerGroup)中的两个Consumer订阅了相同Topic的不同Tag。
...
该例中的错误在于，同一个消费者分组(ConsumerGroup)中的两个Consumer订阅了不同数量的Topic。
...
Consumer在广播消费模式下offset相关数据以json的形式持久化到Consumer本地磁盘文件中，默认文件路径为当前用户主目录下的`.rocketmq_offsets/${clientId}/${group}/Offsets.json`。其中`${clientId}`为当前消费者id，默认为ip@DEFAULT；`${group}`为消费者分组(ConsumerGroup)名称。

### offset远程管理模式

当消费模式为**集群消费**时，offset使用远程模式管理。因为所有Consumer实例对消息采用的是均衡消费，所有Consumer共享Queue的消费进度。
...
- **CONSUME_FROM_TIMESTAMP**：从指定的具体时间戳位置的消息开始消费。这个具体时间戳是通过consumer.setConsumeTimestamp("20210701080000") yyyyMMddHHmmss 指定的。
...
> 第1步已经判断过是否是重复性操作了，为什么第2步还要再次判断？
...
1. 当支付请求到达后，首先在Redis缓存中获取key为支付流水号的缓存value。若value不空，则说明本次支付是重复操作，业务系统直接返回调用侧重复支付标识；若value为空，则进入下一步操作
